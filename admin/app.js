...
`;
  ...
  function renderAnalytics() {
    ...
    document.getElementById("an").textContent = calls.length;
  ...
  function renderTable(rows) {
    ...
    rows.forEach(r => {
      ...
      const st = p > 0
        ? `<span class="status-billable">Billable</span>`
        : `<span class="status-non">Non-billable</span>`;
  ...
  function getStates() {
    const s = {};
    calls.forEach(c => {
      const st = ((c.state || "Unknown").toString().trim().toUpperCase()) || "Unknown";
  ...
  function renderStates() {
    ...
    tb.innerHTML = "";
    if (!st.length) {
      tb.innerHTML = `<tr><td colspan="4" class="text-center py-8" style="color:var(--muted)">No data yet</td></tr>`;
      return;
    }
    st.forEach(s => {
      const tr = document.createElement("tr");
