(function () {
  ...
  // Sheet timestamps are recorded in Pakistan Standard Time (UTC+5, no DST).
  // We recover the literal Y/M/D/H/M/S that were parsed from the sheet string,
  // treat them as PKT, convert to a true UTC instant, then render that instant
  // in US Eastern Time (which correctly handles EST/EDT).
    ...
    try {
      ...
      return rows.slice(1).map(function (r, i) {
        ...
        return {
          ...
          duration: duration,
          billable: getBillableStatus(duration)
        };
      }).filter(function (r) {
  ...
  function updateMetrics(data) {
    ...
    const billableRows = data.filter(function (r) { return r.billable === "billable"; });
    const nonBillableRows = data.filter(function (r) { return r.billable === "nonbillable"; });
  ...
  function applyFilters() {
    ...
    filteredData = rawData.filter(function (r) {
      ...
      if (billableFilter && r.billable !== billableFilter) return false;
      if (from) {
  ...
  function renderTable() {
    ...
    tbody.innerHTML = filteredData.map(function (r) {
      ...
      const isNonBillable = r.billable === "nonbillable";
      return (
  ...
  function exportCSV() {
    ...
    filteredData.forEach(function (r) {
      ...
      const row = [
        formatDate(r.timestamp), r.phone, r.state, r.duration || "", statusLabel,
  ...
  async function loadData() {
    ...
    try {
      ...
      showToast(rawData.length + " record" + (rawData.length !== 1 ? "s" : "") + " loaded successfully");
    } catch (err) {
      console.error(err);
      const tbody = $("#tableBody");
      const empty = $("#emptyState");
